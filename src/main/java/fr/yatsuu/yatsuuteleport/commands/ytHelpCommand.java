...
public class ytHelpCommand implements CommandExecutor {

    private final YatsuuTeleport plugin;

    public ytHelpCommand(YatsuuTeleport plugin) {
    ...
    public boolean onCommand(CommandSender sender, @NonNull Command command, @NonNull String label, @Nonnull String[] args) {
        ...
        if (!sender.hasPermission("yatsuuteleport.command.ythelp")) {

            String no_perm = Objects.requireNonNull(plugin.getConfig().getString("messages.no_perm")).replace("{permission}", "yatsuuteleport.command.ythelp");
